...
function translatePlacePart(text) {
    ...
    for (const pattern of placePatterns) {
        const regex = new RegExp(`([\\u4e00-\\u9fa5]+)${pattern.zh}`, 'g');
        let match;
        while ((match = regex.exec(result)) !== null) {
            const placeName = match[1];
            if (/[\u4e00-\u9fa5]/.test(placeName)) {
                const pinyinName = chineseToPinyin(placeName).split(' ').map(w => capitalizeFirstLetter(w)).join('');
                result = result.replace(match[0], pinyinName + pattern.en);
                regex.lastIndex = 0;
            }
        }
    }
...
function translateProtectUnit(text) {
    ...
    const chineseInSuffix = suffixTranslated.match(/[\u4e00-\u9fa5]+/g);
    if (chineseInSuffix) {
        for (const zh of chineseInSuffix) {
            const pinyinZh = chineseToPinyin(zh).split(' ').map(w => capitalizeFirstLetter(w)).join('');
            suffixTranslated = suffixTranslated.replace(zh, pinyinZh);
        }
    }
...
function translateItemName(name) {
    ...
    if (chineseChars) {
        for (const zh of chineseChars) {
            const pinyinZh = chineseToPinyin(zh).split(' ').map(w => capitalizeFirstLetter(w)).join('');
            result = result.replace(zh, pinyinZh);
        }
    }
...
dataEn.children.forEach(cat => {
    cat.children.forEach(item => {
        ...
        nameFields++;
        if (nameSamples.length < 10 && /[\u4e00-\u9fa5]/.test(item.name)) {
            nameSamples.push({ zh: item.name, en: item.name_en });
        }
        ...
        if (item.records) {
            item.records.forEach(r => {
                ...
                if (r['公布时间']) {
                    ...
                    if (!r.records_en['Publish Date'].includes('Batch')) dateFields++;
                    else dateFields++;
                }
...
dataEn.children.forEach(cat => {
    cat.children.forEach(item => {
        if (item.name_en && /[\u4e00-\u9fa5]/.test(item.name_en)) {
            chineseNames++;
        }
    });
});
...
console.log(`Names with remaining Chinese: ${chineseNames}`);
