...
(function() {
        ...
        modal.innerHTML = `
            <div style="background: #2c241e; border-radius: 24px; padding: 24px; width: 300px; text-align: center; border: 1px solid #c9a96e; box-shadow: 0 20px 30px rgba(0,0,0,0.3);">
                ...
                <div style="display: flex; gap: 12px; justify-content: center;">
                    <button id="confirmOk" style="background: #b22234; color: white; border: none; padding: 8px 20px; border-radius: 40px; cursor: pointer;">确定</button>
                    <button id="confirmCancel" style="background: #666; color: white; border: none; padding: 8px 20px; border-radius: 40px; cursor: pointer;">取消</button>
                </div>
            </div>
        `;
    ...
    function updateAuthArea() {
        ...
        if (currentUser) {
            ...
            let actionButton = '';
            if (isUserPage()) {
                actionButton = `<button id="logoutBtn" class="login-btn" style="background: #666; padding: 6px 16px; border: none; cursor: pointer;">退出</button>`;
            } else {
                actionButton = `<a href="user.html" class="login-btn" style="background: #2c5f2d; padding: 6px 16px;">个人中心</a>`;
            }
            ...
            authArea.innerHTML = `
                <button id="showLoginBtn" class="login-btn" style="background: #b22234; padding: 6px 18px;">登录/注册</button>
            `;
        ...
        if (!modal) {
            ...
            modal.innerHTML = `
                <div class="auth-container">
                    ...
                    <div class="auth-switch-mode" id="authSwitchBtn">没有账号？去注册</div>
                </div>
            `;
            document.body.appendChild(modal);
    ...
    async function verifyPassword(inputPassword, storedPassword, username) {
        ...
        if (isMatch) {
            ...
            if (users[username]) {
                ...
                console.log('[安全] 用户 "' + username + '" 的明文密码已自动升级为 bcrypt 哈希');
            }
        }
        return isMatch;
    }
    ...
    async function handleLogin() {
        ...
        try {
            ...
            console.error('[安全] 密码校验失败:', err);
            showToast('安全组件加载失败，请刷新后重试');
            return;
        }
