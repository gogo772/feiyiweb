        ...
        (function() {
            ...
            const i18n = {
                zh: {
                    ...
                    legendNatural: "自然风光", legendHistoric: "人文古迹", legendIntangible: "非遗体验", legendFood: "美食打卡", legendHighMatch: "高匹配",
                    filterAll: "全部星点", filterNatural: "自然风光", filterHistoric: "人文古迹", filterIntangible: "非遗体验", filterFood: "美食打卡",
                    cult: "文化厚重感", scenery: "风景出片率", food: "美食浓度", intangible: "非遗互动度", leisure: "休闲指数",
                ...
                en: {
                    ...
                    legendNatural: "Nature", legendHistoric: "Heritage", legendIntangible: "ICH", legendFood: "Food", legendHighMatch: "High Match",
                    filterAll: "All Spots", filterNatural: "Nature", filterHistoric: "Heritage", filterIntangible: "ICH", filterFood: "Food",
                    cult: "Cultural Depth", scenery: "Scenic View", food: "Food Scene", intangible: "ICH Experience", leisure: "Relaxation",
            ...
            function updateMapByPreference() {
                ...
                myMapChart.setOption({
                    ...
                    tooltip: { trigger: 'item', backgroundColor: 'rgba(10,20,40,0.9)', borderColor: '#ffc96e', textStyle: { color: '#f8f3e0' } }
                }, false);
            }
            ...
            function updateRadarChart() {
                ...
                myRadarChart.setOption({
                    radar: {
                        ...
                        name: { textStyle: { color: '#EEDDAA' } },
                        splitArea: { areaStyle: { color: ['rgba(255,205,110,0.1)', 'rgba(200,170,90,0.05)'] } }
                    },
                    series: [{
                        type: 'radar',
                        data: [{
                        ...
                        symbol: 'none',
                        lineStyle: { width: 2, color: '#F9D371' },
                        areaStyle: { color: 'rgba(249, 211, 113, 0.25)' }
                    }]
                }, true);
            }

            function refreshAll() { updateRadarChart(); updateMapByPreference(); }
            ...
            function getMapSeries(spots, pref) {
                ...
                // 5) 第一层：普通散点（覆盖全量，统一主色 #d4af37，半透明、默认不显示标签）
                ...
                if (jitteredHigh.length >= 2) {
                    ...
                    for (let i = 0; i < segCount; i++) {
                        linesData.push({
                            ...
                            lineStyle: { color: '#F9D976', width: isLargeMode ? 1.8 : 2.4, curveness: 0.2 }
                        });
                    }
                    // 虚线回环：仅普通模式 + 至少 3 点
                    if (!isLargeMode && pts.length > 2) {
                        linesData.push({
                            ...
                            lineStyle: { color: '#F5B041', width: 1.6, curveness: 0.3, type: 'dashed', opacity: 0.7 }
                        });
                    }
                }
                ...
                const baseEmphasis = {
                    label: { show: true, formatter: '{b}', position: 'top', color: '#FFEDA8', fontSize: 11 },
                ...
                const series = [
                    ...
                    },
                    {
                        ...
                        label: {
                            ...
                            fontWeight: 'bold', color: '#FFEDA8', fontSize: isLargeMode ? 10 : 11,
                            textBorderColor: '#3B2C0A', textBorderWidth: 2
                        },
                ...
                if (linesData.length) {
                    series.push({
                        ...
                        lineStyle: { color: '#FDCF7C', width: isLargeMode ? 1.6 : 2.2, curveness: 0.2, opacity: 0.85 },
                        effect: { show: true, period: isLargeMode ? 6 : 4, trailLength: 0.4, symbol: 'arrow', symbolSize: isLargeMode ? 6 : 8, color: '#FFE5A3' }
                    });
                }
                return series;
            }
            ...
            function getGeoConfig(province) {
                return {
                    map: province.name,                 // echarts.registerMap 用的名称
                    roam: true,
                    zoom: province.zoom || 1.0,
                    center: province.center,
                    itemStyle: { borderColor: '#3B5575', borderWidth: 1.2, areaColor: '#101c34' },
                    emphasis: {
                        itemStyle: { areaColor: '#1f3060' },
            ...
            function refreshMapGeoConfig() {
                ...
                try { currentOption = myMapChart.getOption(); } catch (e) {}
                ...
                myMapChart.on('click', (params) => {
                    if (params.componentType === 'series' && (params.seriesType === 'scatter' || params.seriesType === 'effectScatter')) {
                        ...
                        if (spot) {
                            ...
                            const map = { cultureScore: 'cult', sceneryScore: 'scenery', foodScore: 'food', heritageScore: 'intangible', leisureScore: 'leisure' };
            ...
            const debouncedRefreshAll = debounce(() => { refreshAll(); }, 200);
            ...
            async function switchProvince(provinceCode) {
                ...
                try {
                    ...
                    if (myMapChart) {
                        myMapChart.setOption({
                            geo: {
                                ...
                                label: { show: true, color: '#c0d0e8', fontSize: 10, formatter: cityLabelFormatter },
                                tooltip: { show: true }
                            },
                            backgroundColor: 'transparent',
                    ...
                    applyLanguage();
                    try { localStorage.setItem('appProvince', provinceCode); } catch (e) {}
                    ...
                    setTimeout(() => { setStatus(t.statusReady, ''); }, 1500);
                } catch (err) {
            ...
            function initMapChart() {
                if (!myMapChart) {
                    ...
                    bindMapClick();
                    window.addEventListener('resize', () => { myMapChart && myMapChart.resize(); myRadarChart && myRadarChart.resize(); });
