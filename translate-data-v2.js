...
function translateRegion(text) {
    ...
    for (const [zh, en] of Object.entries(PROVINCE_FULL)) {
        if (result.startsWith(zh)) {
            ...
            result = result.replace(/市辖区/g, ' Urban District');
            ...
            result = result.replace(/乡/g, ' Township');
            result = result.replace(/镇/g, ' Town');
            result = result.replace(/村/g, ' Village');
            result = result.replace(/街道/g, ' Subdistrict');
            result = result.replace(/社区/g, ' Community');
...
dataEn.children.forEach(cat => {
    cat.children.forEach(item => {
        ...
        if (!item.name_en) {
            ...
            nameFields++;
            if (nameSamples.length < 15) nameSamples.push({ zh: item.name, en: item.name_en });
        }
